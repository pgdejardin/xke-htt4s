...
class DoobieBookRepository[F[_]](val xa: Transactor[F])(implicit val ev: Bracket[F, Throwable])
...
object DoobieBookRepository {
  def apply[F[_]: Monad](xa: Transactor[F])(implicit ev: Bracket[F, Throwable]): DoobieBookRepository[F] =
    new DoobieBookRepository(xa)
}
